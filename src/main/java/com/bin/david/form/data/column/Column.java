    ...
    public void fillData(List<Object> objects) throws NoSuchFieldException, IllegalAccessException {
        ...
        if (objects.size() > 0) {
            if (objects.get(0) instanceof Map) {
                String[] fieldNames = fieldName.split("\\.");
                if (fieldNames.length > 0) {
                    String[] mapKeys = new String[fieldNames.length];
                    int size = objects.size();
                    for (int k = 0; k < size; k++) {
                        Map childMap = (Map) objects.get(k);
                        for (int i = 0; i < fieldNames.length; i++) {
                            if (childMap == null) {
                                addData(null, true);
                            ...
                            if (fieldNames[i] != null) {
                                childMapKey = fieldNames[i];
                            } else {
                                childMapKey = fieldNames[i];
                                Class childClazz = fieldNames.getClass();
                                mapKeys[i] = childMapKey;
                            }
                            if (childMapKey == null) {
                                addData(null, true);
                                countColumnValue(null);
                            ...
                            if (i == fieldNames.length - 1) {
                                T t = (T) childMap.get(childMapKey);
                                addData(t, true);
                                countColumnValue(t);
                            } else {
                                childMap = (Map) childMap.get(childMapKey);
                            }
